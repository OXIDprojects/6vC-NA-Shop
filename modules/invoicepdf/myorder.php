...
class PdfArticleSummary extends PdfBlock
{
    ...
    protected function _setTotalCostsWithoutDiscount( &$iStartPos )
    {
        ...
        // BEGIN MOD
        $dSumVat = 0;
        foreach ( $this->_oData->getVats() as $dVatPrice ) $dSumVat += $dVatPrice;
        ...
        $this->text( 45, $iStartPos + 4, ($dSumVat > 0 ? $this->_oData->translate( 'ORDER_OVERVIEW_PDF_ALLPRICENETTO' ) : '') );
        $this->text( 195 - $this->_oPdf->getStringWidth( $sNetSum ), $iStartPos + 4, ($dSumVat > 0 ? $sNetSum : '') );
...
class MyOrder extends MyOrder_parent
{
    ...
    public function pdfFooter( $oPdf )
    {
        ...
        $oPdf->text( 15, 281, strip_tags( $oShop->oxshops__oxcity->getRawValue() ) . ' ' . strip_tags( $oShop->oxshops__oxzip->value ) );
        ...
        $sTxt = ( !empty($oShop->oxshops__oxvatnumber->value) ? $this->translate( 'ORDER_OVERVIEW_PDF_TAXIDNR' ) : '' ).' '.strip_tags( $oShop->oxshops__oxvatnumber->value );
        $oPdf->text( 195 - $oPdf->getStringWidth( $sTxt ), 275, $sTxt );
        $sTxt = ( !empty($oShop->oxshops__oxbanknumber->value) ? $this->translate( 'ORDER_OVERVIEW_PDF_ACCOUNTNR' ) : '' ).strip_tags( $oShop->oxshops__oxbanknumber->value );
    ...
    public function exportStandart( $oPdf )
    {
        ...
        // shop GST#
        $sText = ( empty($oShop->oxshops__oxvatnumber->value) ? ' ' : $this->translate( 'ORDER_OVERVIEW_PDF_TAXIDNR' ) ).' '.$oShop->oxshops__oxvatnumber->value;
        $oPdf->text( 195 - $oPdf->getStringWidth( $sText ), $iTop + 8, $sText );

        // shop PST#
        $sText = ( empty($oShop->oxshops__oxbanknumber->value) ? ' ' : $this->translate( 'ORDER_OVERVIEW_PDF_ACCOUNTNR' ) ).' '.$oShop->oxshops__oxbanknumber->value;
