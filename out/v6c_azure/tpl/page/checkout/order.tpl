[{*--
  ...
  *		Submitting order will direct to merchant gateway if applicable (MerchantLink).
  *		Support for XprsChkOut:
  *			- cannot modify shipping/payment (button conditional)
  *			- back button goes to user page
  *			- errors processing options will prevent display of order button(s)
  *		Remove shipping content for if SellServices installed
--*}]
        ...
        [{block name="checkout_order_details"}]
            ...
            [{else}]
                ...
                [{if $oView->isLowOrderPrice()}]
                    [{ oxmultilang ident="PAGE_CHECKOUT_ORDER_MINORDERPRICE" }] [{ $oView->getMinOrderPrice() }]
                [{elseif $oViewConf->v6cIsMdlInst('XprsChkout') && $oView->v6cIsStopOrderErr()}]
                	[{ oxmultilang ident="V6C_XC_PAGE_CHECKOUT_ORDER_STOPERR" }]
                [{elseif $oView->showOrderButtonOnTop()}]
                    <div class="lineBox clear">
                        <form action="[{ $oViewConf->getSslSelfLink() }]" method="post" id="orderConfirmAgbTop">
                            [{ $oViewConf->getHiddenSid() }]
                            [{ $oViewConf->getNavFormParams() }]
                            [{if $oViewConf->v6cIsMdlInst('merchantlink')}]
                            	<input type="hidden" name="cl" value="[{if $oView->v6cSelPayIsLnkGateway()}]v6c_RedirectPost[{else}]order[{/if}]">
                            [{else}]
                ...
                [{block name="shippingAndPayment"}]
                    ...
                    <form action="[{ $oViewConf->getSslSelfLink() }]" method="post">
                        <h3 class="section">
                            ...
                            <input type="hidden" name="fnc" value="">
                            [{if !$oViewConf->v6cIsXprsChkOut()}]<button type="submit" class="submitButton largeButton">[{ oxmultilang ident="PAGE_CHECKOUT_ORDER_MODIFY2" }]</button>[{/if}]
                        </h3>
                    </form>
                    ...
                    <div id="orderPayment">
                        <form action="[{ $oViewConf->getSslSelfLink() }]" method="post">
                            <h3 class="section">
                                ...
                                <input type="hidden" name="cl" value="payment">
                                <input type="hidden" name="fnc" value="">
                                [{if !$oViewConf->v6cIsXprsChkOut()}]<button type="submit" class="submitButton largeButton">[{ oxmultilang ident="PAGE_CHECKOUT_ORDER_MODIFY3" }]</button>[{/if}]
                            </h3>
                        </form>
                ...
                [{else}]
                    [{block name="checkout_order_btn_confirm_bottom"}]
                        <form action="[{ $oViewConf->getSslSelfLink() }]" method="post" id="orderConfirmAgbBottom">
                            ...
                            <div class="lineBox clear">
                            	[{if $oViewConf->v6cIsXprsChkOut()}]
                            		<a href="[{ oxgetseourl ident=$oViewConf->getSelfLink()|cat:"cl=user" }]" class="prevStep submitButton largeButton">[{ oxmultilang ident="PAGE_CHECKOUT_ORDER_BACKSTEP" }]</a>
                            	[{else}]
