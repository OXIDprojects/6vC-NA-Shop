            ...
            <div class="price [{if $_product->getPricePerUnit()}]tight[{/if}]" id="priceBargain_[{$smarty.foreach.bargainList.iteration}]">
                ...
                [{oxhasrights ident="SHOWARTICLEPRICE"}]
                    ...
                    [{if $_product->getFPrice()}]
                        [{assign var="currency" value=$oView->getActCurrency()}]
                         <span class="priceValue">[{$_product->getFPrice()}] [{if !( $_product->hasMdVariants() || ($oViewConf->showSelectListsInList()&&$_product->getSelections(1)) || $_product->getVariantList() )}][{/if}]</span>
                    [{/if}]
