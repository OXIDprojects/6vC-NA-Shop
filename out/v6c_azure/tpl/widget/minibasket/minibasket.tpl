[{*--
  ...
  *		Support for v6cXprsChkout module (checkout button goes straight to order page)
--*}]
            ...
            [{/if}]
                ...
                <div id="[{$_prefix}]basketFlyout" class="basketFlyout corners[{if $scrollableBasket}] scrollable[{/if}]">
                        ...
                        [{block name="widget_minibasket_product"}]
                            ...
                            <li>
                                <a href="[{$_product->getLink()}]" title="[{ $minibasketItemTitle|strip_tags }]">
                                    ...
                                    <strong class="price">[{$_product->getFTotalPrice()}]</strong>
                                </a>
                            </li>
                        [{/block}]
                    [{/foreach}]
                    ...
                    [{block name="widget_minibasket_total"}]
                        <p class="totals">
                           <span class="item">[{ oxmultilang ident="WIDGET_MINIBASKET_TOTAL" }]</span><strong class="price">[{ $oxcmp_basket->getFProductsPrice()}]</strong>
                        </p>
                    [{/block}]
                    <hr>
                    ...
                    <p class="functions clear">
                       [{if $oxcmp_user}]
                       		[{if $oViewConf->v6cIsXprsChkOut()}]
                       			<a href="[{ oxgetseourl ident=$oViewConf->getSelfLink()|cat:"cl=order&fnc=v6cSetOrderOptions" }]" class="submitButton largeButton">[{ oxmultilang ident="WIDGET_MINIBASKET_CHECKOUT" }]</a>
                       		[{else}]
