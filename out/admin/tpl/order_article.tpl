[{*-- 
  * SUMMARY OF V6C MODS:
  *		Pause & cancel buttons read-only if order already paid.
  *		Change quantity button read-only if order already paid.
--*}]
...
<tr id="art.[{$smarty.foreach.orderArticles.iteration}]">
    ...
    <td valign="top" class="[{ $listclass}]">[{if !$listitem->isBundle()}]<a href="Javascript:DeleteThisArticle('[{ $listitem->oxorderarticles__oxid->value }]');" class="delete" [{if $readonly || $edit->v6cIsPaid() }]onclick="JavaScript:return false;"[{/if}] [{include file="help.tpl" helpid=item_delete}]></a>[{/if}]</td>
    <td valign="top" class="[{ $listclass}]">[{if !$listitem->isBundle()}]<a href="Javascript:StornoThisArticle('[{ $listitem->oxorderarticles__oxid->value }]');" class="pause" [{if $readonly || $edit->v6cIsPaid() }]onclick="JavaScript:return false;"[{/if}] [{include file="help.tpl" helpid=item_storno}]></a>[{/if}]</td>
</tr>
[{if $blWhite == "2"}]
[{assign var="blWhite" value=""}]
[{else}]
...
<input type="submit" value="[{ oxmultilang ident="ORDER_ARTICLE_UPDATE_STOCK" }]" [{if $edit->v6cIsPaid() }]readonly disabled[{/if}]>
